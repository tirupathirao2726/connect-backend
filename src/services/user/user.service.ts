...
export class UserService {
  ...
  async getUserById(userId:string):Promise<ResponseDTO>{
    ...
    try{
      await queryRunner.connect();
      const user = await queryRunner.manager.findOneBy(User,{userId:userId})
      return new ResponseDTO("success",user);
    }
    catch(error){
      return new ResponseDTO('failure',null,`Unable to fetch user with id ${userId}`)
    }
    finally{
      await queryRunner.release();
    }
  }
  async updateUserDetailsById(userId:string,payload:UpdateUserDTO):Promise<ResponseDTO>{
    const queryRunner = dataSource.createQueryRunner();
    try{
      await queryRunner.connect();
      await queryRunner.startTransaction();
      const updateObj = await queryRunner.manager.update(User,{userId},payload);
  ...
  async deleteUser(userId:string):Promise<ResponseDTO>{
    ...
    try{
      ...
      await queryRunner.rollbackTransaction();
      return new ResponseDTO("failure",null,`Unable to delete user ${userId}`);
